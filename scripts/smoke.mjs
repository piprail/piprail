...
console.log(`\n\x1b[1m  SMOKE\x1b[0m  ${selected.length} section(s)${wantMoney ? '  \x1b[33m· L4 WILL SPEND REAL MONEY\x1b[0m' : ''}\n`)
...
for (const L of LAYERS) {
  ...
  for (const m of mine) {
    ...
    const proc = spawnSync('node', [join(SMOKE_DIR, 'run-one.mjs'), m.file, ...wanted], {
    ...
    const pass = sections.reduce((n, s) => n + s.checks.filter((c) => c.status === 'PASS').length, 0)
    const fail = sections.reduce((n, s) => n + s.checks.filter((c) => c.status === 'FAIL').length, 0)
    totalPass += pass
    totalFail += fail
    layerFailed += fail
