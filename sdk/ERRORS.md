...
| `INVALID_CONFIG` | `InvalidConfigError` | invalid merchant gate/manifest config — an invalid `amount` (non-string, or a non-decimal string incl. scientific notation), a missing `payTo`, or a non-array `resources` in `buildWellKnownX402Manifest` | gate setup (`requirePayment`/`createPaymentGate`) · `buildWellKnownX402Manifest` |
