...
Guidance for AI coding agents (and humans) working in this repo. PipRail is an **open, backendless, no-fee** SDK for x402 "402 Payment Required" crypto payments across 30 chains, plus a static marketing site.
...
- **Swap (OPTIONAL, and NOT an agent tool):** `client.quoteSwap({ from, to, wantAmount, slippageBps? })` → `SwapQuote | null` (read-only; never throws for a READ problem — no route, no liquidity, a dead node all return `null`; a malformed `slippageBps` throws `RangeError` before any read) then `client.swap(quote)` → `SwapReceipt`. Nothing swaps by itself: `fetch()` never swaps, `planPayment()` never swaps, and there is **no `autoSwap` flag and no `piprail_swap` tool** — deliberately, because **`policy` does NOT govern swaps** (every cap governs paying a merchant; a swap moves your own funds between denominations). **All 10 driver families can swap**, over 10 routes / 18 of the 30 chains / 21 mainnet proofs — but coverage is per-CHAIN, so on a chain with no route `quoteSwap()` returns `null` and `swap()` throws naming the venues that exist. Two tiers: `source.kind: 'protocol'` = the LEDGER swaps (the **Stellar SDEX** via `PathPaymentStrictReceive` to self, the **XRPL DEX + AMM** via a cross-currency `Payment` to self) with no third party at all; `source.kind: 'provider'` = a named keyless venue (Solana → Jupiter, EVM → KyberSwap, Sui → Aftermath, NEAR → Ref Finance, Algorand → Vestige, Aptos → Hyperion, TON → STON.fi, Tron → SunSwap V2). On **Aptos and Tron there is no API at all** — the quote is an on-chain read, the swap a contract call your key signs. **Every quote carries `source` naming who priced it — PipRail runs no price oracle and takes no fee.** Slippage rides ON-CHAIN (`sendMax`/`SendMax`/`otherAmountThreshold`/`amountInMax`/`amount_in_max`, and TON's `min_ask_units` floor), so a market move fails the tx rather than overspending — nothing is swapped, though EVM/Aptos/Tron still take the gas for a reverted tx. The coverage map is data: `import { SWAP_PROVIDERS } from '@piprail/sdk'`.
