...
**If a facilitator you use disappears**, nothing in PipRail breaks: the facilitator is only involved
in the opt-in gasless `exact` rail, and the default `onchain-proof` rail never touches one.
:::
...
// → 'https://facilitator.payai.network'  (first of PayAI · OpenFacilitator)
