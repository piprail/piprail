...
Build one gate per gated resource and reuse it, because its in-memory used-proof set is what stops a
...
`verify()` returns the discriminated union `VerifyPaymentResult`, so `switch` on `kind`:
...
On `'paid'`, `result.receipt` is the verified settlement, with every field re-derived from the
...
[`onPaid` callback](/accepting-payments/receipts-and-onpaid/), which fires once after verification
succeeds.
...
| `payment_expired` | Older than `maxTimeoutSeconds`, so outside the recency window. |
...
once your node recovers, so the proof is never burned.
:::
...
app.get('/report', async (c) => {
  ...
  try {
    ...
    // gas, facilitator down). NOT the payer's fault, so answer 5xx, never 402.
...
payload shapes; pass it whichever header your client sent, and the `requirePayment` middleware reads
...
throws a `SettlementError` instead of returning `'invalid'`. That's not the payer's fault, since their
