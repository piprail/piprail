...
*actual* usage, then settle exactly that (`actual ≤ max`), billing what was consumed rather than a flat
price.
...
the ceiling, with **zero gas**, broadcasting nothing); you serve + meter; then you **self-settle** the metered
actual through the canonical on-chain `x402UptoPermit2Proxy` **from your own relayer** (which is the bound
`witness.facilitator`, so no third-party facilitator). Backendless, no fee, no custody: the funds land at
your `payTo`, verified against your own RPC.

:::note[EVM-Permit2 only]
`upto` is **EVM-Permit2 only**, because the spec bans EIP-3009 (it fixes the amount at sign time) and has no
...
exact rail, the relayer here **may equal `payTo`**, because the proxy binds `witness.to` (the recipient)
...
settleAmount: (ctx: {
  ...
  asset: string       // the token contract address (or 'native', though native never carries upto)
...
2. **Buyer signs the MAX.** The buyer signs a Permit2 `PermitWitnessTransferFrom` over `amount` as the
   maximum, with **zero gas and no broadcast.** (One-time `approve(Permit2)` aside, the same as the
   exact-Permit2 rail.)
...
4. **`gate.verify()` settles the actual.** The gate verifies the signature against the signed MAX, calls
   ...
   `settle(permit, amount, owner, witness, signature)` from your relayer, where `amount` is the metered
...
- **A genuine broadcast failure is a 5xx.** If your relayer can't broadcast or confirm, the gate throws a
  [`SettlementError`](/errors/error-hierarchy/) (return 5xx in your adapter), so the buyer's authorization
...
- [Pay the upto rail (buyer)](/making-payments/upto-buyer/): the client side, where you opt in, sign the MAX,
