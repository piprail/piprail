...
A2A is an **open protocol**, created by Google and now stewarded by the **Linux Foundation**
...
Google-maintained), which is exactly what the [mount below](#mount-it-in-a2a-jssdk) uses. PipRail's
...
[`PaymentGate`](/accepting-payments/require-payment-and-gate/) you already use for HTTP, with no second
...
[Mount it in `@a2a-js/sdk`](#mount-it-in-a2a-jssdk) for a full, runnable server.
...
carriage are not built, so today your HTTP buyer mints the payload that rides A2A (see
...
`x402Version: 1`) and is currently un-runnable on the live stack, so PipRail deliberately tracks the
...
`node:http`, not a JSON-RPC A2A runtime, so it exercises the handler + the on-chain settle, not the
`@a2a-js/sdk` wiring.)
...
| 2. Buyer submits a proof | `message/send` carrying `x402.payment.payload` | dispatched through `gate.verifyObject` | n/a |
| 3a. **Valid** (`kind: 'paid'`) | n/a | the served artifacts + a success receipt | `completed` · `payment-completed` |
| 3b. **Rejected** (`kind: 'invalid'`) | n/a | a fresh challenge + a failure receipt + error code, **retryable** | `input-required` · `payment-failed` |
| 3c. No usable proof (`kind: 'challenge'`) | n/a | a fresh challenge | `input-required` · `payment-required` |
| 3d. **Settlement error** (`SettlementError` thrown) | n/a | a **terminal** failure | `failed` · `payment-failed` |
| 3e. Settle OK but `fulfill` threw | n/a | the success receipt + an error-annotation artifact | `completed` · `payment-completed` |

Read the pairings carefully, because they're the part most adapters get wrong. The key idea: the
...
- **`input-required` + `payment-required`** is a genuine challenge: the **first** 402 (no payload
...
- **`input-required` + `payment-failed`** is a submitted proof that **failed verification** (expired /
...
- **`completed` + `payment-completed`** means the money moved. The success receipt is in
  `x402.payment.receipts[]` and the served work is in the Task's `artifacts[]`.
- **`failed` + `payment-failed`** is a settlement-side error: the relayer/facilitator never moved
  funds (only on the [`exact` rail](/accepting-payments/exact-rail-seller/)). Same status as a
  rejection, but **terminal** (`state: failed`), because re-submitting the same authorization won't help
...
statuses (the *client* rejecting your offered requirements, or submitting a payload), and a *merchant*
...
You normally never call `verifyObject` yourself, because `handleMessage` does, on the payload it reads out
...
Passing the single shared `gate` avoids the trap entirely, so prefer it. See
...
| `taskStore` | `A2ATaskStore` | A bounded, pluggable TRANSPORT-lifecycle store. It correlates a follow-up `message/send` to its in-flight Task and accumulates `receipts[]`. Default = an in-memory TTL Map. NOT on the verification path. |
...
A fulfill artifact is `{ name, parts: [{ kind: 'text', text }] }`, so the content lives under `parts`,
