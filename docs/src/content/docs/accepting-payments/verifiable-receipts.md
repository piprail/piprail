...
reconciliation** side, where `onPaid` fires *inside your server* so you can fulfil the order and persist
...
:::note[Tier 2 degrades gracefully and can never fail your 200]
...
- **`info.receipt`** is the **spec slot**: the official `SignedReceipt` (`{ format, payload,
  ...
  When there's no attestation (Tier-1 only), `info.receipt` is absent, so a stock reader correctly sees
...
PipRail defaults `includeTxHash` to **`true`**, so the settlement tx hash is in the receipt, which makes it
...
:::caution[Payer-privacy trade-off, so decide deliberately]
...
tx to re-read), so pair it with a **Tier-2 attestation**, which still proves delivery against your
...
The other families (EVM, Solana, Tron, Sui, Aptos, and every native coin, the "Template B" set) are
digest-bound: they verify on the `transaction` alone, so for them the `nonce` is purely
informational. You don't configure any of this, because it's automatic, but it's why a receipt from a
...
- [Receipts & onPaid](/accepting-payments/receipts-and-onpaid/): the *delivery* side, persisting a
