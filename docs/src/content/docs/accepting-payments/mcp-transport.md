...
Don't confuse this with **[`@piprail/mcp`](/mcp/overview/)**, which is PipRail's standalone MCP *server*
...
[`PaymentGate`](/accepting-payments/require-payment-and-gate/) you already use for HTTP and A2A, with no
...
etc.). Passing the single shared `gate` avoids the trap entirely, so prefer it. See
...
For hand-built adapters, the low-level codec is exported too, all pure and all chain-agnostic:
