...
isolated, may be sync or async, and can deliver receipts durably. All of that is covered below.
...
a rejected promise** is caught and routed to `onPaidError`, so it can never break the request,
...
Without `onPaidError`, a failing receipt hook is swallowed silently (the safe default, since it
...
- **`awaitOnPaid: true`** makes the gate await the hook before serving the resource, so "receipt
...
A stateless gate can't be a durable webhook on its own, which is exactly why `deliverReceipt`
...
and **upsert on the `idempotency-key` header**, because `deliverReceipt`'s retries and at-least-once
...
**either** the v2 `payment-signature` header or the legacy v1 `x-payment` header, and the gate
parses both:
...
returns `502` on a `SettlementError` itself, so you only reach for `buildReceiptHeader` and the
...
**submitted proof is rejected**, meaning [`gate.verify()`](/accepting-payments/verifying-payments/)
...
exactly like `onPaid`, and it fires through both, because it fires *inside* `gate.verify()`:
...
interface FailedPayment {
  ...
  transient: boolean      // true for the two transient codes, where the proof may still be settling
}
```
...
| `transient` | `true` only for the two transient codes `tx_not_found` / `insufficient_confirmations`, where the proof may still be settling and the buyer auto-retries. `false` for a definitive rejection the buyer must fix. See below. |
...
thrown error **or a rejected promise** is caught and routed to `onFailedError`, so it can never break
...
break a request). Without it, a failing `onFailed` hook is swallowed silently, so set it to log or
...
| `verify()` **throws**, on a transient RPC error re-throw or a 5xx [`SettlementError`](/accepting-payments/exact-rail-seller/) | no | A throw isn't a payment *verdict*; it's an infrastructure fault. The gate returns 5xx, the buyer's signed authorization stays valid + unused, and there's nothing to record as a buyer failure. |
...
facilitator is down) does **not**, because it's your infrastructure, not the buyer's payment. See
...
Both `requirePayment` and `createPaymentGate` fire `onFailed` identically, because it lives inside
