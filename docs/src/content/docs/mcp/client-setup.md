...
the raw key into the config file, so treat that file as a secret.
:::
...
CLI with `claude mcp add piprail --scope user --env PIPRAIL_CHAIN=base -- npx -y @piprail/mcp`,
...
`env` block and treat the file as a secret. You can also `clawhub install piprail` to discover it. Full
