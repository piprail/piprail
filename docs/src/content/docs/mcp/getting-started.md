...
facilitator, and PipRail never touches your funds.
...
Desktop does **not**, so treat that config file as a secret). The server **never** accepts the key
as a CLI argument, because `argv` leaks in process listings and shell history.
:::
...
:::tip
EVM chains work out of the box, because the package ships with `viem`, so `base`, `ethereum`,
...
:::note
It's not either/or. The MCP server *is* the SDK with a config layer, and the env block becomes the
...
| --- | --- | --- | --- |
| `PIPRAIL_PRIVATE_KEY` | only to **pay** | none | The wallet, in the chain's native key format. Omit it to run **read-only** (discover/quote/register/budget/guide). |
