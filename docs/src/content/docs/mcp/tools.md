...
| `network` | string | CAIP-2 id, `'self'` (your chain, the default), or `'any'` (all chains). |
...
| `minReliability` | number | Drop results below this health score (0 to 100); unscored results pass through. |
...
:::tip
Discovery results are cross-scheme. Always call `piprail_quote_payment` on a chosen resource, which
...
Check whether you **can** pay before committing. It reads your wallet balance, native gas, and
...
crashing. The fields are **mutually contextual**, populated by the kind of failure, so
...
| `declined` | policy / approval refusal | `true`, and **no funds moved**. |
...
be on-chain**, so never re-pay or you double-spend. The first two carry the broadcast proof on
...
`MAX_RETRIES_EXCEEDED`). The MCP exposes only the buyer side, and `onFailed` is a server-gate
...
| `category` | string | **The top findability lever**, since most listings have none. A real category (`'ai'`, `'finance'`, …) makes a listing rank + filter. |
...
gate gets listed but they can't pay it, so add an [`exact` rail](/accepting-payments/exact-rail-seller/)
...
`grandTotal` + `counts` resume after a restart, still a caller-owned file with no backend.
:::
...
Re-verify a PipRail **verifiable receipt** against the chain, confirming a payment **really** settled
...
When the server is built with `guide` on (the default, since `PIPRAIL_GUIDE` off only suppresses it),
