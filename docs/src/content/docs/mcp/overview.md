...
token, `USDC` — so the only truly required field is `PIPRAIL_PRIVATE_KEY` (alias `AGENT_KEY`).
...
:::caution
Treat the config file as a secret — it holds a private key. Use `${env:…}` interpolation where
...
Only `piprail_pay_request` ever moves money; everything else is read-only, and each tool carries
MCP annotations so your client can show the right consent. The spend policy — not the annotations
— is the real boundary. See the [Tools reference](/mcp/tools/) for inputs, outputs, and errors.

## Two modes

- **Mode A — headless (default).** The agent runs free *inside* the budget and time envelope.
  The policy **is** the consent — no per-payment prompt. Over-budget requests are refused before
  any on-chain send.
- **Mode B — supervised (`PIPRAIL_CONFIRM=1`).** Each payment asks the human for approval at the
  ...
  or error means **not** paying. Mode B sits *on top of* the policy — it never replaces it.
...
- The spend policy (`PIPRAIL_MAX_AMOUNT` / `PIPRAIL_MAX_TOTAL` / `PIPRAIL_TOKENS` /
  ...
  (the SDK's, never a server's) — a server can't slip past a cap by understating a price.
