...
rail (≈ \$1) but **1 whole native coin** on a `native` rail, which is ~\$1000s for ETH, not \$1. There's
...
Adding 1 USDC to 1 SOL is unit-meaningless without a price oracle, which
doesn't ship. Each token gets its own running cap. One consequence for multi-chain: the **same
token across N chains gets N independent caps**, so `maxTotal: '20.00'` with USDC on Base *and*
...
- **`chains`** match the configured selector on string entries (an EVM preset like `'base'` or a
  family name like `'solana'`); object selectors (a viem `Chain` or `{ id, rpcUrl }`) match by
  resolved network id.
- **`tokens`** are matched against the token's **true symbol**. The special value `'native'` is a
  chain-agnostic alias for the chain's native coin, matching ETH, BNB, TRX, XLM, and so on
  without naming the ticker, mirroring the merchant-side `token: 'native'`.
- **`hosts`** are exact (`api.example.com`) or wildcard (`*.example.com`, which also matches the
...
if (!quote) {
  // not payment-gated, so nothing to check
...
When the policy refuses, the typed `policyCode` says exactly which guard fired, with no prose-parsing
...
try {
  ...
  if (err instanceof PaymentDeclinedError) {
    // No funds moved. Branch on the typed reason, and never re-pay on a terminal code.
...
(An `onBeforePay` decline, described below, has no `policyCode` and reports `reasonCode: 'APPROVAL'`.)
...
const client = new PipRailClient({
  ...
  // amountFormatted is a string, so compare it as a number, not lexicographically.
...
A `PaymentPolicy` has **one** `maxAmount`/`maxTotal` pair, so on its own it can't express a different
...
```ts
// per-payment ceiling, keyed by the REAL ticker (the native coin shows as its symbol, e.g. ETH on Base)
