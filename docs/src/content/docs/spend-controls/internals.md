...
into the ledger after. `evaluatePolicy()` itself is pure (no clock, no I/O) — the client injects a
...
### 2. Amounts are non-negative integers — negatives/decimals/junk are rejected
The wire amount must match `^\d+$`. A negative (`"-5"`), fractional (`"0.5"`), exponential (`"1e6"`),
or non-numeric amount is a [`InvalidEnvelopeError`](/errors/error-hierarchy/) — it can never reach the
...
### 4. A denomination cap can't be ESCAPED with deep decimals — fail closed
