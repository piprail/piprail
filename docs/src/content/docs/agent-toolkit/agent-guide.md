...
description: PIPRAIL_AGENT_GUIDE is the full agent payment contract distilled into one string you can drop into an LLM system prompt.
sidebar:
  order: 4
---
...
It's a pure static constant, with no imports and no I/O, so you can prepend it to any agent's system
prompt, MCP or not.
...
told not to attempt the payment, and `fundingHint` says exactly what to fix.
...
| Rail | What it does |
| --- | --- |
| `onchain-proof` | PipRail's default. The buyer broadcasts the payment itself and pays the network gas (the native coin, so ETH/SOL/…). Works on every chain. |
...
| --- | --- |
| `declined` + `SESSION_EXPIRED` | **Terminal.** Time budget over. Stop, and retry no payment this process. |
...
| `PAYMENT_TIMEOUT` / `MAX_RETRIES_EXCEEDED` / `CONFIRMATION_TIMEOUT` | The payment may already be on-chain, so **never re-pay** (it would double-spend). |
...
before reading it. `ConfirmationTimeoutError` carries **no** `.ref`, so there the recovery is to
...
read back. It is read-only and moves no funds. See [the agent tools](/agent-toolkit/the-agent-tools/)
...
- **Mode A (headless, default):** the agent runs free inside a pre-set budget + [time
  ...
  the consent, so there is no per-payment prompt. `piprail_budget` shows what's left.
- **Mode B (supervised):** the host may ask a human to approve each payment. A
  decline/cancel/timeout comes back as `declined: true` with `reasonCode: 'APPROVAL'`, so the model
...
- **Per-payment + per-`(network, asset)` caps always apply.** On top of them an *optional*
  ...
  stablecoin + chain") sums tokens declared as one unit, each 1:1. It is **not** a price oracle, and it
...
- **The time envelope lives in-memory for this process** and resets on restart. The money + count
  totals also reset on restart **unless a durable spend store is configured**, in which case they resume.
