...
`paymentTools()` accepts any `PayingClient`, either a single-chain `PipRailClient` **or** a
...
`schemes: ['onchain-proof', 'exact']` lets the tools pay the ratified x402 `exact` rail, where the buyer
...
`invoke` always resolves to a JSON-serialisable value, and it does not throw for a payment
...
about a tool's *nature*. They are hints only, so never make a security decision on them. The
