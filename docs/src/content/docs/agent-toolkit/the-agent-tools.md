...
Only one of the eight moves funds; the other seven are read-only or write to an external index, and
none of them can spend. The model can't bypass the spend policy either, because every payment routes
...
(never fiat, because PipRail has no price oracle). The `session` time leash is added only when a time
...
`onchain-proof` (PipRail's backendless default, where the buyer broadcasts and pays gas), or, when the
...
`destructiveHint: true`, and `idempotentHint: false`, because a payment is value-moving and not
...
```jsonc
// piprail_budget()   takes no arguments
...
```jsonc
// piprail_guide()   takes no arguments
...
Re-verifies a PipRail **verifiable receipt** against the chain, confirming a payment *really*
...
throws**, because there's no wallet and no payment, so a chain or RPC failure is reported in `error`
