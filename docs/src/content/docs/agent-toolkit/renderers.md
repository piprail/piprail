...
chain's native coin (ETH/SOL/TRX/…), never in fiat, because PipRail has no price oracle. This is the
...
```text
… Recover using the proof on .ref (re-verify or re-submit it); never re-pay, because a fresh payment would double-spend.
```
...
**no `.ref`**, so re-check the proof ref you already broadcast. The rendered line states the rule in
...
**no price oracle**: a denomination total is a *unit-of-account sum* that adds up every token the
...
`FDUSD`, `RLUSD`, `U` fold into `USD`, `EURC` into `EUR`) and is extensible per client via
