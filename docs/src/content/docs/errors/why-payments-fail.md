...
so you recover with the proof and never re-pay.
...
| Code | Class | The fix |
| --- | --- | --- |
| `INSUFFICIENT_FUNDS` | `InsufficientFundsError` | Fund the **payer** with more of the payment token, or native coin for gas/reserve. |
| `RECIPIENT_NOT_READY` | `RecipientNotReadyError` | Set up the **recipient** (`payTo`). It isn't provisioned to receive on this chain yet. |

Affordability always converges on one error on every chain, `InsufficientFundsError` with `.code ===`
...
| Algorand | a one-time **ASA opt-in** for the issued token (e.g. USDC), which is a 0-amount self-transfer. Native ALGO needs nothing. |
...
`SESSION_EXPIRED` and `APPROVAL` are terminal, so every further payment this process is refused;
...
| `UNSUPPORTED_SCHEME` | Asked to pay a scheme the bound family/asset/signer can't settle, with no fallback. For example an [`exact`](/making-payments/exact-buyer/) rail on a family without an exact implementation (TON, Tron, Sui, Stellar; EVM, Solana, Algorand, Aptos, NEAR and XRPL DO support it), a non-EIP-3009 token (USDT, plain ERC-20), a contract / EIP-1271 / EIP-7702 signer, or an XRPL **issued currency** such as RLUSD (native XRP is exact-payable; issued currencies state a decimal wire amount the spend cap can't price, so they stay on `onchain-proof`). Note the native coin is exact-payable on XRPL and **only** there. |
...
split, so don't conflate them. See [Chains and tokens](/concepts/chains-and-tokens/) and
...
**never re-broadcasts**. If it still can't confirm, it throws one of these, and the first two carry
...
| `MAX_RETRIES_EXCEEDED` | `MaxRetriesExceededError` | Broadcast, retried, still 402. The message embeds the last server `error` and `detail`. Carries `.ref`. |
...
`authorizationState(from, nonce)`; the non-EVM rails key off their own marker (Solana uses the buyer's
