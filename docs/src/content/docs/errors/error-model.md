...
| | Channel | Shape | For |
| --- | --- | --- | --- |
| 1 | **THROWN** | a typed [`PipRailError`](/errors/error-hierarchy/) subclass with a stable `.code` | config / flow / wallet / registry / affordability, the things you act on |
...
try {
  ...
  if (err instanceof PipRailError && err.code === 'PAYMENT_DECLINED') {
    // over policy / refused before any send, so nothing moved
  } else throw err
}
```
...
throw inside either is caught and routed to its `…Error` seam, so neither can break the request.
...
try {
  ...
  if (err instanceof PipRailError && err.code === 'INSUFFICIENT_FUNDS') {
    // fund the payer with more token, native gas, or reserve
  } else throw err
}
```
...
## Recovering a broadcast proof, and never re-paying
...
`'SESSION_EXPIRED'` and `'APPROVAL'` are **terminal**, so every payment this process makes is now
...
`wallet`, since they read YOUR balance, gas, and recipient-readiness), plus
