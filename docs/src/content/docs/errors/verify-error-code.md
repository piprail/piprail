...
The set is **closed**, and the compiler enforces it, so a driver can't invent a code, and every
...
| `upto_settle_exceeds_max` | The [`upto` (metered) rail](/accepting-payments/upto-rail-seller/): the merchant's metered settle amount exceeds the maximum the buyer signed (`permitted.amount`). Gate/driver-enforced before any broadcast, so nothing settles. | definitive |
...
`false` for a definitive rejection. Alert the merchant on `!transient`, because a transient code usually
...
- **`tx_not_found`** is the only transient that all drivers emit. The proof ref didn't resolve to
  a transaction on the merchant's RPC: it hasn't landed yet, or the read itself failed.
- **`insufficient_confirmations`** means the tx is mined but not yet `minConfirmations` deep. Emitted
  by families with a discrete confirmation depth (EVM and XRPL); the client retries after the backoff.
- **`tx_reverted`** means the transaction exists on chain but its execution failed, so nothing settled.
- **`no_meta`** is Solana-specific: the transaction returned no metadata to inspect, so the transfer
  can't be read.
- **`wrong_recipient`** means a digest-bound transfer landed, but not to the merchant's `payTo`.
- **`amount_too_low`** means the transfer reached `payTo` but paid less than the rail required.
- **`transfer_not_found`** means no transfer matching the asset, amount, and nonce was found on
  `payTo`. On account-watch chains this also absorbs "wrong recipient" (see below).
- **`payment_expired`** means the proof is older than the rail's `maxTimeoutSeconds` recency window. On
...
- **`tx_already_used`** is the verify-style code the gate emits for the onchain-proof replay set, and
  that the EVM `exact` / Permit2 driver also returns via the token's on-chain `authorizationState` /
  Permit2 nonce check.
- **`signature_invalid`** is `exact`-rail only: the signed authorization didn't validate against the
