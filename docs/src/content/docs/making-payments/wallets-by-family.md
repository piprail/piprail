...
`key` always holds that chain's standard secret string, and the SDK validates the format for the
chain you named:
...
On EVM you can hand the client an injected viem `walletClient` instead of a raw key, so the visitor
...
// The SDK needs a walletClient with an ATTACHED account, so connect first.
...
The error is raised lazily, because the driver auto-mounts and binds the wallet on the first call (see
