...
`onchain-proof` rail is unaffected, because the payer's tx creates the recipient ATA, so it's always a
safe fallback.
:::
...
`*_UNREADABLE` / `*_UNKNOWN` warning, and **never a false `0`**. PipRail won't tell you you're
...
(or `true` when the URL isn't gated, because a free resource is trivially affordable):
...
merged `PaymentPlan | null`, with `null` only when the URL isn't gated for any client:
...
Gas is shown in the chain's native coin (ETH/SOL/TRX/…), never in fiat, because PipRail has no price
