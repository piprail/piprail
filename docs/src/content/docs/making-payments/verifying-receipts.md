...
description: 'Capture the receipt from a paid fetch (lastReceipt) and re-verify ANY PipRail receipt against the chain: static and wallet-free.'
sidebar:
  order: 11
---
...
## Capture with `client.lastReceipt()`
...
`lastReceipt()` is **pure**, with no chain read. The client captures the receipt from the
...
## Re-verify Tier-1 with `PipRailClient.verifyReceipt()`
...
- **`ok`** is the chain's confirmation that the settlement is real: at least `amount` of `asset`
...
- **`onChain.payer`** is the one field **genuinely re-derived from the tx** (the rest are pinned by
...
- **`onChain.amount`** is a **verified lower bound**, not a re-derived exact figure. Drivers
  threshold-check `paid >= required`, then echo the accept amount, so `ok: true` means *at least* this
  much moved.
- **`ageSeconds`** is informational only. It is **never** a validity gate (verifyReceipt uses a
  ~100-year synthetic window precisely so a driver's `payment_expired` branch can't fire on a
  legitimately old receipt).
- **`error`** is a closed [`VerifyErrorCode`](/errors/verify-error-code/) (e.g. `tx_not_found`,
...
from the receipt's payer + tx automatically, so you pass nothing.)
:::
...
**account-watch** families (**Stellar, XRPL, Algorand, TON**), whose drivers scan only the merchant
...
## Re-verify Tier-2 with `PipRailClient.verifyAttestation()`
...
- **`ok: true`** with **`signer`** means the recovered address equals `payTo`; delivery is attested by the
...
// 2) Re-verify Tier-1 against the chain. It re-derives, and never trusts the claims.
const v = await PipRailClient.verifyReceipt(receipt)
...
chain-recovered verdict, with no wallet required.

## See also

- [Verifiable receipts (seller)](/accepting-payments/verifiable-receipts/): the emit side, turning
