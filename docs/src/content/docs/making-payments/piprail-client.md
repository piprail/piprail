...
- [`quote()`](/making-payments/quote/) — learn the price without paying it.
- [`estimateCost()`](/making-payments/estimate-cost/) — add a best-effort native-coin gas
  estimate so you budget payment **+** gas.
- [`planPayment()`](/making-payments/plan-payment/) — the full readiness check across every rail
...
:::note
The ledger is in-memory and **process-scoped** — it resets on restart. For crash-resistant
