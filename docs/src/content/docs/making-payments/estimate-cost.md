...
but it **never pays**, and it returns `null` when the URL isn't payment-gated.
...
| `basis` | Meaning |
| --- | --- |
| `'estimated'` | Derived from a live RPC read (EVM gas price, XRPL fee), so it's sharp. |
...
if (result && result.cost.basis === 'heuristic') {
  // the gas figure is a ballpark, so keep a wider native-coin margin
}
```
...
:::tip
On a standard `exact` rail (EIP-3009), the buyer's gas is estimated as **~0**, because the merchant's
