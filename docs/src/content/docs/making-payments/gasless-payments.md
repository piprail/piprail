...
description: 'How gasless x402 payments work in PipRail: the two rails (onchain-proof vs the gasless exact rail), and the exact rail''s six auto-selected methods.'
sidebar:
  order: 9
---
...
- A 402 is paid over one of **two rails**:
  - **`onchain-proof`** is PipRail's **default**. *You* broadcast the transfer, so **you pay the gas**.
    Works on every chain. (The "with-gas" path.)
  - **`exact`** is the ratified x402 rail (**opt-in**). You only **sign**; someone else broadcasts, so
    **you pay zero gas**. This is the gasless rail.
- **`exact` has six *methods***, meaning *how* the signature is shaped, **auto-selected** per chain + token.
  These are children of `exact`, **not** alternatives to it:
  - **EIP-3009** covers EVM tokens with `transferWithAuthorization` (USDC, EURC). The clean path.
  - **Permit2** covers EVM tokens *without* EIP-3009 (e.g. Binance-Peg USDC on BNB). Self-settle only.
  - **SVM** covers Solana, any SPL token.
  - **Algorand** covers any ASA (USDCa), via an atomic-group **fee pool** (the buyer's transfer rides at fee 0).
  - **Aptos** covers any Fungible Asset (USDC, USDT), via a native **fee-payer (sponsored) transaction** (AIP-39).
  - **NEAR** covers NEP-141 tokens (USDC, USDT), via a **NEP-366 meta-transaction** (the buyer signs at 0 NEAR; a relayer pays). Self-settle only.
...
| Buyer pays gas? | **yes** (a normal transfer) | **no**, it just signs |
...
**Once you opt in, yes. It just works, and you never wire up the gasless mechanics by hand.** The opt-in
...
Which one to trust is your call (some need an API key). After that one
...
- **Buyer:** add `'exact'` to `schemes`. The client then signs (never broadcasts), so it spends **zero
  ...
  [`autoRoute`](/making-payments/fetch-and-autoroute/), which pays the **cheapest settleable** rail, and that is the
...
- **Seller:** the one-liner is **`exact: true`**. The gate **auto-picks a known keyless facilitator**
  for each offered chain (from `KNOWN_FACILITATORS`) and routes settlement to it, so **neither buyer nor
  ...
  gas) with a **loud warning**, and it never bricks the gate. Pin `exact: { settle: { facilitator } }` to
...
**EIP-3009 is the cleanest EVM path**: no approval, no extra contract, and the buyer needs only the
stablecoin. **Permit2 covers the EVM gap**, meaning tokens that *don't* implement EIP-3009 (most notably the
...
(the fee txn is separate from the transfer, so there is no isolation rule), so a merchant can be its own relayer.
Native **ALGO** isn't exact-payable (the scheme is an ASA transfer), so it stays on `onchain-proof`. (See
...
**one-shot** (the buyer needs only the advertised `feePayer`, with no gas-station round-trip). So **any
...
can be its own relayer. Native **APT** isn't exact-payable, so it stays on `onchain-proof`. (See the
...
It's **self-settle only** today, because no keyless facilitator settles NEAR yet, so on NEAR `exact: true`
degrades to `onchain-proof` while `exact: { settle: 'self', relayer }` enables the rail. Native **NEAR**
isn't exact-payable (the scheme is an NEP-141 transfer), so it stays on `onchain-proof`.

## ⭐ Which chains & tokens are gasless?

**Gasless has two strengths, so be precise about which one a chain has:**

| Tier | Who pays gas | When | How |
|---|---|---|---|
| 🟢 **Both-sides-gasless** | **Nobody.** A **keyless third-party facilitator** sponsors it, so buyer **and** merchant pay zero. | A keyless facilitator is seeded for the chain | **zero-config**: `exact: true` |
...
implements EIP-3009, so naming the chain is all it takes, with no proxy and no approval. Each chain above
...
`payTo`**, and the recipient's **token account must already exist**, because the exact rail won't create it
...
**opted into the ASA** to receive it, which `recipientReady()` checks. Native **ALGO** isn't
...
`feePayer === payTo` is allowed (the merchant can sponsor its own receive). It's **one-shot**, with no
...
"open rail, no lock-in" guarantee for one chain's gasless convenience. It is not a gap in the research.
...
chain, so **neither buyer nor merchant pays gas**, with no relayer key and no facilitator URL:
...
with a loud warning, and it never bricks the gate. For more control, pin a specific facilitator (recommended
...
gas**, since the facilitator does). No relayer key, no SOL:
...
**Pieverse**), so neither buyer nor merchant pays gas (live-settled on BNB mainnet: FDUSD via Dexter, and
**U via Pieverse** 2026-06-21, with buyer *and* merchant paying 0 BNB). USDC/USDT (Permit2) stay
**self-settle only** (your relayer pays the gas). One caveat: Dexter enforces a **~$0.003 dynamic floor**
on BNB, so a sub-$0.003 payment is rejected, which is fine for real prices. This is the BNB token-overlap wall
...
The fee payer must never appear in any instruction, never be invoked as a
source of funds (and it must differ from `payTo`). The recipient's token account must already exist (the
exact rail never creates it, which keeps the transaction to the strict canonical form; a brand-new
...
(the scheme is defined over SPL `TransferChecked`), so it stays on `onchain-proof`.
...
Solana gate; point `settle.facilitator` at whichever you trust. For the **full cross-chain list**, including Base-only
...
group integrity atomically at `simulate`/submit. Unlike Solana, **`feePayer === payTo` is allowed**, so the
...
Like Algorand, **`feePayer === payTo` is allowed**, because the fee-payer signature is a separate authenticator,
...
| **EVM** (EIP-3009 / Permit2) | nothing buyer-controlled | none *(structurally immune)* | n/a |
...
TransferChecked, Memo]`, where the buyer adds a spec-required SPL-Memo (the rail's `extra.memo`, else a random
...
1. **Discovers the sponsor.** On the fee-payer rails (**Solana, Algorand, Aptos**) it reads the
   ...
   sponsor address, because the relayer is implicit.) Pin it with `settle: { facilitator, feePayer }` to skip
   this lookup.
2. **Settles.** It POSTs the buyer's signed authorization to the facilitator's `/verify`, then `/settle`; the
...
client echo, so the facilitator only broadcasts and never gets to redefine the payment. You can verify any
...
|---|---|---|---|
| A facilitator is down **at challenge time**, on a *multi-rail* gate | The exact rail for that chain is **dropped**; the gate still serves `onchain-proof` (+ any working rails) | none *(challenge still succeeds)* | **Automatic.** The buyer pays `onchain-proof`. |
| No chain can carry exact, on **`exact: true`** (the soft shorthand): no keyless facilitator seeded for the chain, a native coin, etc. | **Graceful degrade.** The gate serves `onchain-proof` **only** (never bricks) + a **loud warning** naming the cause (fires once at startup, **in production too**, because a silent prod degrade is the trap; silence with `PIPRAIL_NO_HINTS=1`) | none *(challenge succeeds; buyers pay gas via `onchain-proof`)* | **Automatic.** The warning shows the remedy; for guaranteed gasless, pin `settle:{facilitator}` or use `settle:'self'`. |
| No chain can carry exact, on **explicit** `settle: 'self'` / `{ facilitator }` (a deliberate gasless intent that can't be honoured) | The gate **throws** loud at construction, because an explicit config error is a bug to surface rather than silently downgrade | `requirePayment: exact was requested but none of the offered rails support it …` | Fix the config (typo'd URL, wrong/native token), **or** switch to the soft `exact: true` (which degrades), **or** drop `exact` (the `onchain-proof` default always works). |
| The facilitator **errors at settle** (down / 401 / timeout) | The signed authorization stays **valid + unused**, so there is no double-spend | **HTTP 502** `{ error: 'settlement_failed', detail, fallback }`, where the `fallback` field **explicitly names `onchain-proof`** as the pay-gas retry, *not* a 402 | Re-present the **same** payment when it recovers, **or** pay `onchain-proof` on the same endpoint *(verified: 502 → onchain-proof → 200)*. |
| The facilitator **rejects** the payment (bad sig / expired / insufficient) | A conformant re-challenge, with **no spend recorded** | **HTTP 402** + a mapped `VerifyErrorCode` and `Facilitator rejected the payment: <reason>` | The agent reads the reason, fixes it, re-presents the **same** authorization. |
...
2. **Fall back to `onchain-proof`.** It needs no facilitator; the buyer broadcasts and pays sub-cent gas. Always available, every chain.
