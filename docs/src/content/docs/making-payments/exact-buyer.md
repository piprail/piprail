...
this rail existed, because `exact` is strictly opt-in.
:::
...
gas-funded key at all, so settlement is fully gasless end to end (see
...
- **`eip3009`** (EVM) covers canonical USDC/EURC and other tokens exposing `transferWithAuthorization`. The
...
- **`permit2`** (EVM) covers any ERC-20 **without** EIP-3009, most notably **Binance-Peg USDC/USDT on BNB
...
- **`svm`** (Solana) covers **any** SPL token (USDC, USDT, …). The client builds the SPL `TransferChecked`
  ...
  no proxy, no approval, and gasless for the buyer regardless of token. See
  [Gasless payments](/making-payments/gasless-payments/).
- **`algorand`** (Algorand) covers **any** ASA (USDCa, …). The client signs an ASA transfer at **fee 0**,
  atomically grouped with the sponsor's fee-pooling `pay`; the sponsor signs that fee txn and submits
  the group. No token feature required, and gasless for the buyer regardless of token.
- **`aptos`** (Aptos) covers **any** Fungible Asset (USDC, USD₮, …). The client signs a fee-payer
  (sponsored, AIP-39) `primary_fungible_store::transfer` (sender slot only); the sponsor adds the
  fee-payer signature and submits. No token feature required, and gasless for the buyer regardless of token.
- **`near`** (NEAR) covers **any** NEP-141 token (USDC, USDT). The client signs a NEP-366
...
- **`sequence`** (XRP Ledger) covers **native XRP**. It is the odd one out twice over. First, the method names
...
| Solana SVM, any SPL token (USDC / USDT) | XRPL **issued** currencies (RLUSD and co, with decimal wire amounts) |
...
An `exact` rail is selected only when the 402 names a network **your bound chain supports**, and the
...
Solana, Algorand, Aptos, NEAR and Hedera schemes never define the key at all, and their required extra
...
all is paid as `eip3009`, and the buyer re-derives the token's EIP-712 domain on-chain anyway, so the
...
**1. The payer pays the fee, so `exact` is not gasless here.** On every other family the merchant or a
...
says so outright: `extra.areFeesSponsored` **must** be `false`. You still only sign, and the merchant
...
| field | value |
| --- | --- |
| `Amount` | the rail's `amount`, verbatim, as an **integer drops** string |
...
| `LastLedgerSequence` | derived from the rail's own `maxTimeoutSeconds` (~4s a ledger, clamped to 5 to 60 ledgers) |
| `Flags` | `tfFullyCanonicalSig`, and never `tfPartialPayment` |
| `Memos`, `Paths`, `DeliverMin`, `DeliverMax`, `SendMax`, `Delegate`, `NetworkID` | **absent**, because the scheme has settlers reject them |
...
our payload with `invalid_payload`, from their own pre-check, so their facilitator never sees it. The
...
if (!plan) {
  await client.fetch(url)        // not gated, so fetch it for free
...
than signing something that can't settle. (A non-EIP-3009 ERC-20 is **not** in this list, because it pays
via Permit2; nor is an SPL token on Solana, which pays via SVM.)
...
try {
  ...
  if (err instanceof UnsupportedSchemeError) {
    // this chain/asset/signer can't pay the exact rail, so fall back to onchain-proof
...
- A **transport error or timeout** after the authorization is submitted throws
  [`PaymentTimeoutError`](/errors/error-hierarchy/) carrying the nonce as `.ref`, so the facilitator
...
**same** authorization, and never re-sign:

On the non-EVM `exact` rails the `.ref` is likewise the family's single-use marker, recovered
differently. **Solana** uses the buyer's transaction signature (a duplicate signature is the chain's
