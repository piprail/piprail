...
lowercase, base64-JSON header, with no `X-` prefix.
...
`exact` rail, `X402ExactAcceptEntry`, mirrors it with an `extra.assetTransferMethod` instead, which is
...
payload references the on-chain transaction as `{ nonce, txHash }`: the challenge nonce plus the
...
- **`exact`** (opt-in): the ratified x402 `exact` scheme, meaning EIP-3009 signed authorizations on EVM,
