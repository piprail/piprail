...
:::tip
On **BNB Chain**, Binance-Peg USDC/USDT are **18 decimals**, not 6. The preset carries the
correct decimals, so reading from `CHAINS.bnb.tokens` (or naming `token: 'USDC'`) is always
right — don't hardcode 6.
:::
