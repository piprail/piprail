...
nor its `viem` peer becomes a Mastra dependency, which is the cleanest possible footprint. The integration *is* the
...
as **`piprail_piprail_*`** (server key + tool name), which is standard Mastra behavior. The agent reads exact
...
| --- | --- | --- | --- |
| `PIPRAIL_PRIVATE_KEY` | only to **pay** | none | Self-custodial wallet key/seed (EVM `0x…`, Solana base58, or a mnemonic). **Omit for read-only.** Not an API key, and never commit it. |
| `PIPRAIL_CHAIN` | no | `base` | Chain to pay on (any EVM, or `solana`/`ton`/`tron`/`near`/`sui`/`aptos`/`algorand`/`stellar`/`xrpl`). |
| `PIPRAIL_CHAINS` | no | none | **Multi-chain.** Comma-separated; each takes its own `PIPRAIL_<CHAIN>_KEY`. Pays whichever chain a 402 asks for. |
| `PIPRAIL_MAX_AMOUNT` | no | `0.10` | Max per payment, in the token's units (≈ $ for USDC/USDT). |
| `PIPRAIL_MAX_TOTAL` | no | `10.00` | Lifetime budget per token (server default; this example sets `5.00`). |
