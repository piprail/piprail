...
:::tip[elizaOS has a sell-side x402 plugin, and this is the buy side]
...
**Wallet required.** The plugin needs `PIPRAIL_PRIVATE_KEY` (a self-custodial key), and *every* action
...
| --- | --- | --- | --- |
| `PIPRAIL_PRIVATE_KEY` | **yes** | none | Self-custodial wallet key (EVM `0x…`, Solana base58, …). Put it in `settings.secrets`. Every action requires it. Not an API key. |
| `PIPRAIL_CHAIN` | no | `base` | Which chain to pay on: any EVM, or `solana`/`ton`/`tron`/`near`/`sui`/`aptos`/`algorand`/`stellar`/`xrpl`. |
| `PIPRAIL_MAX_AMOUNT` | no | `0.10` | Max per payment → `policy.maxAmount` |
| `PIPRAIL_MAX_TOTAL` | no | `5.00` | Lifetime budget → `policy.maxTotal` |
