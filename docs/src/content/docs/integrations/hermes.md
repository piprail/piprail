...
Nous Portal is fiat-subscription only, and there's even an open RFC for an agent wallet
...
`PIPRAIL_PRIVATE_KEY` **must** be in the server's `env:` block, or pay stays `WALLET_REQUIRED`.
Keep the literal key in `~/.hermes/.env` (chmod 600) and reference it as `${PIPRAIL_PRIVATE_KEY}`, because
...
`hermes skills tap add piprail/skills`. The skill only teaches the quote → plan → pay workflow, so
...
| `PIPRAIL_CHAINS` | no | none | **Pay across several chains** (instead of `PIPRAIL_CHAIN`), e.g. `base,polygon,solana`, each with its own `PIPRAIL_<CHAIN>_KEY`; pays whichever chain a 402 asks for. See [Configuration](/mcp/configuration/#pay-on-several-chains-from-one-server). |
