...
The quickstart runs in one process with sensible in-memory defaults. Going to production, whether
a fleet of agents, a load-balanced gate, or a long-lived service, means knowing exactly **what is
...
| [Time envelope](/spend-controls/time-envelope/) (TTL, rolling window) | per process, because the session *is* the process | a restart begins a fresh window |
...
- **Dedupe on `receipt.idempotencyKey`** (a unique index / upsert), because across instances the same
...
field, so **fold any key into the URL**. Public RPCs are rate-limited and will throttle a busy gate
...
`onchain-proof`-only gate gets *listed* but those clients **can't pay it**, which is a discoverable dead
