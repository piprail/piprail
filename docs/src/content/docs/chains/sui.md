---
title: "Accept x402 USDC payments on Sui"
...
`@mysten/sui` is an optional peer, so install it only on the chains you actually touch. See the
...
A Sui wallet is `{ key }`, either a `suiprivkey1…` bech32 secret or the raw 32-byte secret as a
...
here and the format is validated by `Ed25519Keypair`, so a hex `0x…` EVM key won't parse and
...
Wormhole-bridged token that is not Circle/Tether-redeemable, so it's intentionally absent. The native
...
Sui has no protocol-level swap, so a named venue routes it:
**[Aftermath](https://aftermath.finance)**. It is keyless, and PipRail sets no integrator or
platform fee on it.
