...
Tron is the single largest stablecoin-payment rail on earth, holding roughly 45% of all USDT
...
| `{ address, decimals }` | Any other TRC-20 | n/a | Base58 `T…` contract, passed by you. |
...
## Gas is real money, so budget TRX
...
:::tip
For tiny test sends, rent Energy from a service like TronZap or feee.io for ~1 to 2 TRX instead of
...
[`InsufficientFundsError`](/errors/error-hierarchy/) (`.code === 'INSUFFICIENT_FUNDS'`), whose
...
`Transfer` event; a native TRX payment is a plain `TransferContract`. Same path, just reading
...
:::note
Tron's CAIP-2 id (`tron:mainnet`) is internal, because Tron isn't in the official x402 CAIP-2 registry.
