...
The XRP Ledger (XRPL) is a non-EVM family that's payment-native: 3 to 5s single-step finality, no
...
characters, which USDC and RLUSD both are, use the 160-bit hex form.
:::
...
XRPL uses [Template A](/concepts/proof-binding/): the challenge nonce rides in a **Memo**, which is
