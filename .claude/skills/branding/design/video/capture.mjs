...
const format = process.env.FORMAT || 'wide';
const SCENE = pathToFileURL(join(HERE, 'scene.html')).href + (format === 'wide' ? '' : '?portrait');
...
const page = await browser.newPage({ viewport: { width: format === 'wide' ? 1920 : 1080, height: format === 'vertical' ? 1920 : 1080 }, deviceScaleFactor: 2 });
await page.goto(SCENE, { waitUntil: 'load' });
await page.evaluate(() => window.__ready);
await page.evaluate(({format}) => { if(format !== 'wide') { const s=document.getElementById('stage'); s.style.width='1080px'; s.style.height=format==='vertical'?'1920px':'1080px'; } }, {format});
await page.waitForTimeout(400); // let webfonts settle

const pad = (n, w = 5) => String(n).padStart(w, '0');

if (mode === 'sample') {
  const dir = join(HERE, 'sample-' + format);
  ...
  const dir = join(HERE, 'frames-' + format);
