...
const SCENE = pathToFileURL('/Users/john/Sites/piprail/.claude/skills/branding/design/video/scene.html').href;
...
if (mode === 'sample') {
  mkdirSync('/Users/john/Sites/piprail/.claude/skills/branding/design/video/sample', { recursive: true });
  const times = args.length ? args.map(Number) : [1,5,9,11,14,16,20,24,29,31,33,35];
  for (const t of times) {
    await page.evaluate((tt)=>window.seek(tt), t);
    await page.screenshot({ path: `/Users/john/Sites/piprail/.claude/skills/branding/design/video/sample/t_${String(t).replace('.','_')}.png` });
    process.stdout.write(`sample t=${t} `);
  }
  console.log('\nsamples done');
} else {
  const fps = Number(args[0]||30);
  const dir = '/Users/john/Sites/piprail/.claude/skills/branding/design/video/frames';
